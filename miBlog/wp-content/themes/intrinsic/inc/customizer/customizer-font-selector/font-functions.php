 ...
 * @since 1.0
 */
function intrinsic_get_standard_fonts() {
	return apply_filters(
		'intrinsicstandard_fonts_array', array(
 ...
 * @since 1.0
 */
function intrinsic_enqueue_google_font( $font ) {
	...
	$subsets     = '';
	$get_subsets = intrinsic_get_options( array('font_subsets', array( 'latin' ) ) );
	...
	// Weights
	$weights = apply_filters( 'intrinsic_font_weights', array( '200', '300', '400', '500', '600', '700', '800' ) );
	...
	// Enqueue style
	wp_enqueue_style( 'intrinsic-google-font-' . $handle, $url, array(), false );
}

function intrinsic_enqueue_google_fonts_url( $font ) {
	// Get list of all Google Fonts
	$google_fonts = intrinsic_get_google_fonts_array();
