...
class OneClickDemoImport {
  ...
  private $plugin_page_setup = array();
		...
		if ( $this->plugin_page === $hook || ( 'admin.php' === $hook && $this->plugin_page_setup['menu_slug'] === esc_attr( $_GET['import'] ) ) ) {
			...
			wp_localize_script( 'ocdi-main-js', 'ocdi',
				array(
					...
					'texts'            => array(
						'missing_preview_image' => esc_html__( 'No preview image defined for this import.', 'intrinsic' ),
						'dialog_title'          => esc_html__( 'Are you sure?', 'intrinsic' ),
						'dialog_no'             => esc_html__( 'Cancel', 'intrinsic' ),
						'dialog_yes'            => esc_html__( 'Yes, import!', 'intrinsic' ),
						'selected_import_title' => esc_html__( 'Selected demo import:', 'intrinsic' ),
					),
	...
	public function import_demo_data_ajax_callback() {
		...
		if ( ! $use_existing_importer_data ) {
			...
			if ( ! empty( $_FILES ) ) { // Using manual file uploads?
				...
				$this->import_files[ $this->selected_index ]['import_file_name'] = esc_html__( 'Manually uploaded files', 'intrinsic' );
			}
			elseif ( ! empty( $this->import_files[ $this->selected_index ] ) ) { // Use predefined import files from wp filter: pt-ocdi/import_files.
				...
				if ( is_wp_error( $this->selected_import_files ) ) {
					...
					Helpers::log_error_and_send_ajax_response(
						...
						$this->log_file_path,
						esc_html__( 'Downloaded files', 'intrinsic' )
					);
				}
				...
				$log_added = Helpers::append_to_file(
					sprintf(
						__( 'The import files for: %s were successfully downloaded!', 'intrinsic' ),
				...
				wp_send_json( esc_html__( 'No import files specified!', 'intrinsic' ) );
			}
		}
	...
	private function final_response() {
		...
		if ( empty( $this->frontend_error_messages ) ) {
			...
			if ( ! apply_filters( 'pt-ocdi/disable_pt_branding', false ) ) {
				$twitter_status = esc_html__( 'Just used One Click Demo Import plugin and it was awesome! Thanks', 'intrinsic' );
				
			}

			$response['message'] .= sprintf(
				__( '%1$s%3$sThat\'s it, all done!%4$s%2$sThe demo import has finished. Please check your page and make sure that everything has imported correctly. %3$s%4$s.%5$s', 'intrinsic' ),
			...
			$response['message'] .= sprintf(
				__( '%1$sThe demo import has finished, but there were some import errors.%2$sMore details about the errors can be found in this %3$s%5$slog file%6$s%4$s%7$s', 'intrinsic' ),
	...
	public function load_textdomain() {
		//load_plugin_textdomain( 'intrinsic', false, plugin_basename( dirname( dirname( __FILE__ ) ) ) . '/languages' );
	}
