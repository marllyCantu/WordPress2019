	...
	public static function import( $customizer_import_file_path ) {
		...
		if ( is_wp_error( $results ) ) {
			...
			Helpers::append_to_file(
				$error_message,
				$log_file_path,
				esc_html__( 'Importing customizer settings', 'intrinsic' )
			);
		}
		else {
			// Add this message to log file.
			$log_added = Helpers::append_to_file(
				esc_html__( 'Customizer settings import finished!', 'intrinsic' ),
				$log_file_path,
				esc_html__( 'Importing customizer settings' , 'intrinsic' )
			);
		}
	...
	public static function import_customizer_options( $import_file_path ) {
		...
		if ( ! file_exists( $import_file_path ) ) {
			return new \WP_Error(
				...
				sprintf(
					esc_html__( 'Error: The customizer import file is missing! File path: %s', 'intrinsic' ),
					$import_file_path
				)
			);
		}
		...
		if ( ! is_array( $data ) && ( ! isset( $data['template'] ) || ! isset( $data['mods'] ) ) ) {
			return new \WP_Error(
				...
				esc_html__( 'Error: The customizer import file is not in a correct format. Please make sure to use the correct customizer import file.', 'intrinsic' )
			);
		}
		if ( $data['template'] !== $template ) {
			return new \WP_Error(
				'customizer_import_wrong_theme',
				esc_html__( 'Error: The customizer import file is not suitable for current theme. You can only import customizer settings for the same theme or a child theme.', 'intrinsic' )
			);
		}
