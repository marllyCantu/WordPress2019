...
class WPCLICommands extends \WP_CLI_Command {
	...
	public function list_predefined() {
		if ( empty( $this->ocdi->import_files ) ) {
			WP_CLI::error( esc_html__( 'There are no predefined demo imports for currently active theme!', 'intrinsic' ) );
		}

		WP_CLI::success( esc_html__( 'Here are the predefined demo imports:', 'intrinsic' ) );
	...
	public function import( $args, $assoc_args ) {
		if ( ! $this->any_import_options_set( $assoc_args ) ) {
			WP_CLI::error( esc_html__( 'At least one of the possible options should be set! Check them with --help', 'intrinsic' ) );
		}
	...
	private function import_predefined( $predefined_index ) {
		if ( ! is_numeric( $predefined_index ) ) {
			WP_CLI::error( esc_html__( 'The "predefined" parameter should be a number (an index of the OCDI predefined demo import)!', 'intrinsic' ) );
		}

		$predefined_index = absint( $predefined_index );

		if ( ! array_key_exists( $predefined_index, $this->ocdi->import_files ) ) {
			WP_CLI::warning( esc_html__( 'The supplied predefined index does not exist! Please take a look at the available predefined demo imports:', 'intrinsic' ) );
		...
		WP_CLI::log( esc_html__( 'Predefined demo import started! All other parameters will be ignored!', 'intrinsic' ) );

		$selected_files = $this->ocdi->import_files[ $predefined_index ];

		if ( ! empty( $selected_files['import_file_name'] ) ) {
			WP_CLI::log( sprintf( esc_html__( 'Selected predefined demo import: %s', 'intrinsic' ), $selected_files['import_file_name'] ) );
		}

		WP_CLI::log( esc_html__( 'Preparing the demo import files...', 'intrinsic' ) );

		$import_files =	Helpers::download_import_files( $selected_files );

		if ( empty( $import_files ) ) {
			WP_CLI::error( esc_html__( 'Demo import files could not be retrieved!', 'intrinsic' ) );
		}

		WP_CLI::log( esc_html__( 'Demo import files retrieved successfully!', 'intrinsic' ) );

		WP_CLI::log( esc_html__( 'Importing...', 'intrinsic' ) );
		...
		WP_CLI::log( esc_html__( 'Predefined import finished!', 'intrinsic' ) );
	}
	...
	private function import_content( $relative_file_path ) {
		...
		if ( ! file_exists( $content_import_file_path ) ) {
			WP_CLI::warning( esc_html__( 'Content import file provided does not exist! Skipping this import!', 'intrinsic' ) );
			return false;
		}
		...
		WP_CLI::log( esc_html__( 'Importing content (this might take a while)...', 'intrinsic' ) );

		Helpers::append_to_file( '', $this->ocdi->log_file_path, esc_html__( 'Importing content' , 'intrinsic' ) );
		...
		if( empty( $this->ocdi->frontend_error_messages ) ) {
			WP_CLI::success( esc_html__( 'Content import finished!', 'intrinsic' ) );
		}
		else {
			WP_CLI::warning( esc_html__( 'There were some issues while importing the content!', 'intrinsic' ) );
	...
	private function import_widgets( $relative_file_path ) {
		...
		if ( ! file_exists( $widgets_import_file_path ) ) {
			WP_CLI::warning( esc_html__( 'Widgets import file provided does not exist! Skipping this import!', 'intrinsic' ) );
			return false;
		}

		WP_CLI::log( esc_html__( 'Importing widgets...', 'intrinsic' ) );

		WidgetImporter::import( $widgets_import_file_path );

		if( empty( $this->ocdi->frontend_error_messages ) ) {
			WP_CLI::success( esc_html__( 'Widgets imported successfully!', 'intrinsic' ) );
		}
		else {
			WP_CLI::warning( esc_html__( 'There were some issues while importing widgets!', 'intrinsic' ) );
	...
	private function import_customizer( $relative_file_path ) {
		...
		if ( ! file_exists( $customizer_import_file_path ) ) {
			WP_CLI::warning( esc_html__( 'Customizer import file provided does not exist! Skipping this import!', 'intrinsic' ) );
			return false;
		}

		WP_CLI::log( esc_html__( 'Importing customizer settings...', 'intrinsic' ) );

		CustomizerImporter::import( $customizer_import_file_path );

		if( empty( $this->ocdi->frontend_error_messages ) ) {
			WP_CLI::success( esc_html__( 'Customizer settings imported successfully!', 'intrinsic' ) );
		}
		else {
			WP_CLI::warning( esc_html__( 'There were some issues while importing customizer settings!', 'intrinsic' ) );
	...
	private function do_action( $action, $import_files = array(), $all_import_files = array(), $selected_index = null ) {
		if ( false !== has_action( $action ) ) {
			WP_CLI::log( sprintf( esc_html__( 'Executing action: %s ...', 'intrinsic' ), $action ) );
